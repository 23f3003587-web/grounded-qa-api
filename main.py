...
Answer strictly from the given context chunks only.
If answer not found → answer="I don't know", answerable=false, citations=[]
Output ONLY valid JSON: {"answer": "...", "citations": ["C1"], "confidence": 0.95, "answerable": true}
Be very concise.
"""
...
async def grounded_qa(req: Request):
    ...
    try:
        ...
        citations = [cid for cid in data.get("citations", []) 
                     if isinstance(cid, str) and cid in {c.chunk_id for c in req.chunks}]
...
# GET support for judge
@app.get("/grounded-qa")
@app.get("/")
async def grounded_qa_get():
    return {
        "status": "ok",
        "message": "POST method required for QA. See /docs"
    }
